...
export class ListingDeleted extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ListingDeleted entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type ListingDeleted must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`
      );
      store.set("ListingDeleted", id.toString(), this);
    }
  }

  static load(id: string): ListingDeleted | null {
    return changetype<ListingDeleted | null>(store.get("ListingDeleted", id));
  }
